...
(function($) {
  ...
  Canvas.prototype.getTextInfo = function(layer, text, font, angle, width) {
    ...
    if (info == null) {
      ...
      var textWidth = element.outerWidth(true), textHeight = element.outerHeight(true);
      ...
      if (angle) {
        ...
        var radians = angle * Math.PI / 180, sin = Math.sin(radians), cos = Math.cos(radians);
        var a = cos.toFixed(6), // Use fixed-point so these don't
        ...
        if ($.support.leadingWhitespace) {
          ...
          var cx = textWidth / 2, cy = textHeight / 2, transformOrigin = Math.floor(cx) + "px " + Math.floor(cy) + "px";
        ...
        var ac = Math.abs(cos), as = Math.abs(sin), originalWidth = textWidth;
  ...
  function Plot(placeholder, data_, options_, plugins) {
    ...
    function processData() {
      var topSentry = Number.POSITIVE_INFINITY, bottomSentry = Number.NEGATIVE_INFINITY, fakeInfinity = Number.MAX_VALUE, i, j, k, m, s, points, ps, val, f, p, data, format;
    ...
    function measureTickLabels(axis) {
      ...
      tickWidth = opts.tickWidth || opts.labelWidth || 0, tickHeight = opts.tickHeight || opts.labelHeight || 0, maxWidth = tickWidth || axis.direction === "x" ? Math.floor(surface.width / (ticks.length || 1)) : null, layer = "flot-" + axis.direction + "-axis flot-" + axis.direction + axis.n + "-axis " + axis.direction + "Axis " + axis.direction + axis.n + "Axis", font = opts.tickFont || "flot-tick-label tickLabel";
    ...
    function allocateAxisBoxFirstPhase(axis) {
      ...
      if (axisOptions.label) {
        var layer = "flot-" + axis.direction + "-axis flot-" + axis.direction + axis.n + "-axis " + axis.direction + "Axis " + axis.direction + axis.n + "Axis", font = axisOptions.labelFont || "flot-axis-label axisLabels " + axis.direction + axis.n + "axisLabel", angle = axis.direction === "x" ? 0 : axisOptions.position === "right" ? 90 : -90, labelInfo = surface.getTextInfo(layer, axisOptions.label, font, angle);
    ...
    function setupGrid() {
      var axes = allAxes(), showGrid = options.grid.show, margin = options.grid.margin || 0, i, a;
    ...
    function draw() {
      ...
      // Prepare surface restricted to plot Area for drawing series
      ctx.save();
    ...
    function drawAxisLabels() {
      ...
      $.each(allAxes(), function(_, axis) {
        ...
        var box = axis.box, axisOptions = axis.options, layer = "flot-" + axis.direction + "-axis flot-" + axis.direction + axis.n + "-axis " + axis.direction + "Axis " + axis.direction + axis.n + "Axis", labelFont = axisOptions.labelFont || "flot-axis-label axisLabels " + axis.direction + axis.n + "axisLabel", tickFont = axisOptions.tickFont || "flot-tick-label tickLabel", tick, x, y, halign, valign;
    ...
    function drawSeriesLines(series) {
      ...
      function plotLineArea(datapoints, axisx, axisy) {
        var points = datapoints.points, ps = datapoints.pointsize, bottom = Math.min(Math.max(0, axisy.min), axisy.max), i = 0, areaOpen = false, ypos = 1, segmentStart = 0, segmentEnd = 0;
    ...
    function insertLegend() {
      ...
      var fragments = [], entries = [], rowStarted = false, lf = options.legend.labelFormatter, s, label, i;
    ...
    function findNearbyItem(mouseX, mouseY, seriesFilter) {
      var maxDistance = options.grid.mouseActiveRadius, smallestDistance = maxDistance * maxDistance + 1, item = null, i, j, ps;
      ...
      for (i = series.length - 1; i >= 0; --i) {
        ...
        my = axisy.c2p(mouseY), maxx = maxDistance / axisx.scale, maxy = maxDistance / axisy.scale, x, y;
  ...
  $.plot = function(placeholder, data, options) {
    ...
    // (window.console ? console.log : alert)("time used (msecs): " + ((new Date()).getTime() -
    // t0.getTime()));
    return plot;
  };
