...
(function ($) {
  ...
  function Plot (placeholder, data_, options_, plugins) {
    ...
    function adjustLayoutForThingsStickingOut () {
      ...
      $.each(allAxes(), function (_, axis) {
        ...
        if (axis.reserveSpace) {
          margins[dir] = Math.ceil(Math.max(margins[dir], (dir === "x" ? axis.labelWidth
              : axis.labelHeight) / 2));
        }
      });
    ...
    function setRange (axis) {
      var opts = axis.options, min = +(opts.min !== null ? opts.min : axis.datamin), max = +(opts.max !== null ? opts.max
      ...
      if (opts.alignTicksWithAxis !== null) {
        ...
        if (otherAxis && otherAxis.used && otherAxis !== axis) {
          ...
          if (!axis.mode && opts.tickDecimals === null) {
            var extraDec = Math.max(0, -Math.floor(Math.log(axis.delta) / Math.LN10) + 1), ts = axis
                .tickGenerator(axis);
    ...
    function drawBackground () {
      ...
      ctx.fillStyle = getColorOrGradient(options.grid.backgroundColor, plotHeight, 0,
          "rgba(255, 255, 255, 0)");
    ...
    function drawSeriesLines (series) {
      ...
      function plotLineArea (datapoints, axisx, axisy) {
        var points = datapoints.points, ps = datapoints.pointsize, bottom = Math.min(Math.max(0,
            axisy.min), axisy.max), i = 0, top, areaOpen = false, ypos = 1, segmentStart = 0, segmentEnd = 0;
    ...
    function drawSeriesPoints (series) {
      ...
      if ((lw > 0) && (sw > 0)) {
        ...
        ctx.lineWidth = w;
        ctx.strokeStyle = "rgba(0,0,0,0.1)";
        plotPoints(series.datapoints, radius, null, w + w / 2, true, series.xaxis, series.yaxis,
            symbol);
      ...
      ctx.lineWidth = lw;
      ctx.strokeStyle = series.color;
      plotPoints(series.datapoints, radius, getFillStyle(series.points, series.color), 0, false,
          series.xaxis, series.yaxis, symbol);
      ctx.restore();
    }

    function drawBar (x, y, b, barLeft, barRight, fillStyleCallback, axisx, axisy, c, horizontal,
        lineWidth) {
    ...
    function drawSeriesBars (series) {
      function plotBars (datapoints, barLeft, barRight, fillStyleCallback, axisx, axisy) {
        ...
        for ( var i = 0; i < points.length; i += ps) {
          ...
          drawBar(points[i], points[i + 1], points[i + 2], barLeft, barRight, fillStyleCallback,
      ...
      plotBars(series.datapoints, barLeft, barLeft + series.bars.barWidth, fillStyleCallback,
    ...
    function insertLegend () {
      ...
      for ( i = 0; i < entries.length; ++i) {
        ...
        fragments.push('<td class="legendColorBox"><div style="border:1px solid ' + options.legend.labelBoxBorderColor +
            ...
            ';overflow:hidden"></div></div></td>' + '<td class="legendLabel">' + entry.label +
            '</td>');
      }
        ...
        if (s.bars.show && !item) { // no other point can be nearby
          ...
          for (j = 0; j < points.length; j += ps) {
            ...
            if (series[i].bars.horizontal ? ((mx <= Math.max(b, x)) &&
                ...
                (my <= y + barRight)) : ((mx >= x + barLeft) && (mx <= x + barRight) && (my >= Math.min(b, y)) && (my <= Math.max(b, y)))) {
              item = [i, j / ps];
            }
          }
        }
