...
(function($) {
  ...
  function init(plot) {
    ...
    var canvas = null, target = null, maxRadius = null, centerLeft = null, centerTop = null, processed = false, ctx = null;
      ...
      function drawPie() {
        ...
        if (options.series.pie.stroke.width > 0) {
          ...
          for ( var j = 0; j < slices.length; ++j) {
            drawSlice(slices[j].angle, options.series.pie.stroke.color, false);
          }
          ctx.restore();
        }
        ...
        function drawLabels() {
          ...
          function drawLabel(slice, startAngle, index) {
            ...
            if (options.series.pie.label.background.opacity !== 0) {
              ...
              // put in the transparent background separately to avoid blended labels and label
              // boxes
    ...
    function drawDonutHole(layer) {
      if (options.series.pie.innerRadius > 0) {
        ...
        layer.globalCompositeOperation = "destination-out"; // this does not work with excanvas, but
        // it will fall back to using the stroke
        // color
        layer.beginPath();
    ...
    function findNearbySlice(mouseX, mouseY) {
      ...
      for ( var i = 0; i < slices.length; ++i) {
        ...
        if (s.pie.show) {
          ...
          ctx.beginPath();
          ctx.moveTo(0, 0); // Center of the pie
          // ctx.scale(1, options.series.pie.tilt); // this actually seems to break everything when
          // here.
          ...
          if (ctx.isPointInPath) {
            ...
            // TODO: perhaps do some mathmatical trickery here with the Y-coordinate to compensate
            // for pie tilt?
    ...
    function drawOverlay(plot, octx) {
      ...
      function drawHighlight(series) {
        ...
        // octx.fillStyle = parseColor(options.series.pie.highlight.color).scale(null, null, null,
        // options.series.pie.highlight.opacity).toString();
        octx.fillStyle = "rgba(255, 255, 255, " + options.series.pie.highlight.opacity + ")"; // this
        // is
        // temporary
        // until
        // we
  ...
  var options = {
    series : {
      pie : {
        show : false,
        radius : "auto", // actual radius of the visible pie (based on full calculated radius if
        // <=1, or hard pixel value)
        ...
        label : {
          ...
          radius : 1, // radius at which to place the labels (based on full calculated radius if
          // <=1, or hard pixel value)
          background : {
            color : null,
            opacity : 0
